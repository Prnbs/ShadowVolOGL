...
void Cube::Create()
{
    ...
    //bunny's scale
   // ScaleMatrix(&ModelMatrix, 400.01f, 400.01f, 400.01f);
    CubeRotation = 0;
...
//	cout << "Edge table size = " << edgeTable.size() << endl;
    ...
    for(it = edgeTable.begin(); it != edgeTable.end(); it++)
    {
        ...
        TranslatePointOnVector(*(it->second.first), lightPos, result, state.extrudedLen);
        ...
        TranslatePointOnVector(*(it->second.second), lightPos, result, state.extrudedLen);
        ShadowVertices.push_back(*result);
        indexCtr += 4;
